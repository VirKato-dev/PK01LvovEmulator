...
int     r_B, r_C, r_D, r_E, r_H, r_L, r_A, r_F, r_PC, r_SP, clock,
        cpu_halt_reason;
...
static
{
    int i, j; flags = new int[256];
    for (i=0; i<256; i++)
    {
        flags[i] = f_1 | f_P;
        for (j=0; j<8; j++) if ((i & (1<<j)) != 0) flags[i] ^= f_P;
        if ((i & 0x80) !=0 ) flags[i] |= f_S;
        if (i == 0) flags[i] |= f_Z;
   }
}
...
I8080()
{
    ...
    cpu_halt_state = false; cpu_halt_reason = halt_hlt;
...
private final boolean signal_inv_opc()
{
    ...
    if (halt_if_invalid) cpu_halt_state = true;
...
void eval(int ticks)
{
    int opcode, tmp1, tmp2, tmp3; int upto = clock+ticks;
...
////////
        if (r_PC == bpx)
        {
            cpu_halt_state = true; cpu_halt_reason = halt_bpx; break;
        }
        opcode = do_read(r_PC); // if do_read() causes halt it 'll be handled
        if (cpu_halt_state || clock > upto) break;
...
case 0x02: // 02 STAX B
    ...
    break;
case 0x03: // 03 INX B
    if (++r_C > 0xFF) {r_C = 0; if (++r_B > 0xFF) r_B = 0;};
    clock += 5;
    r_PC += 1;
    break;
case 0x04: // 04 INR B
    r_F = flags[tmp2 = (r_B + 1) & 0xFF] | (r_F & f_C);
    if (((tmp2 ^ r_B) & 0x10) != 0) r_F |= f_A;
    r_B = tmp2;
    clock += 5;
    r_PC += 1;
    break;
...
case 0x07: // 07 RLC
    r_A <<= 1;
    if (r_A > 0xFF) {r_A |= 1; r_F |= f_C; r_A &= 0xFF;} else r_F &= ~f_C;
    clock += 4;
    r_PC += 1;
    break;
case 0x08: // 08 _nop_
...
case 0x09: // 09 DAD B
    r_L += r_C;
    r_H += r_B;
    if (r_L > 0xFF) {r_H++; r_L &= 0xFF;};
    if (r_H > 0xFF) {r_H &= 0xFF; r_F |= f_C;} else r_F &= ~f_C;
    clock += 10;
    r_PC += 1;
    break;
case 0x0A: // 0A LDAX B
    ...
    break;
case 0x0B: // 0B DCX B
    if (--r_C < 0) {r_C = 0xFF; if (--r_B < 0) r_B = 0xFF;};
    clock += 5;
    r_PC += 1;
    break;
case 0x0C: // 0C INR C
    r_F = flags[tmp2 = (r_C + 1) & 0xFF] | (r_F & f_C);
    if (((tmp2 ^ r_C) & 0x10) != 0) r_F |= f_A;
    r_C = tmp2;
    clock += 5;
    r_PC += 1;
    break;
...
case 0x0F: // 0F RRC
    tmp1 = r_A & 1;
    r_A >>= 1;
    if (tmp1 != 0) {r_A |= 0x80; r_F |= f_C;} else r_F &= ~f_C;
    clock += 4;
    r_PC += 1;
    break;
...
case 0x12: // 12 STAX D
    ...
    break;
case 0x13: // 13 INX D
    if (++r_E > 0xFF) {r_E = 0; if (++r_D > 0xFF) r_D = 0;};
    clock += 5;
    r_PC += 1;
    break;
case 0x14: // 14 INR D
    r_F = flags[tmp2 = (r_D + 1) & 0xFF] | (r_F & f_C);
    if (((tmp2 ^ r_D) & 0x10) != 0) r_F |= f_A;
    r_D = tmp2;
    clock += 5;
    r_PC += 1;
    break;
...
case 0x17: // 17 RAL
    r_A <<= 1;
    if ((r_F & f_C) != 0) r_A |= 1;
    if (r_A > 0xFF) {r_F |= f_C; r_A &= 0xFF;} else r_F &= ~f_C;
    clock += 4;
    r_PC += 1;
    break;
case 0x19: // 19 DAD D
...
case 0x1A: // 1A LDAX D
    ...
    break;
case 0x1B: // 1B DCX D
    if (--r_E < 0) {r_E = 0xFF; if (--r_D < 0) r_D = 0xFF;};
    clock += 5;
    r_PC += 1;
    break;
case 0x1C: // 1C INR E
    r_F = flags[tmp2 = (r_E + 1) & 0xFF] | (r_F & f_C);
    if (((tmp2 ^ r_E) & 0x10) != 0) r_F |= f_A;
    r_E = tmp2;
    clock += 5;
    r_PC += 1;
    break;
...
case 0x1F: // 1F RAR
    tmp1 = r_A & 1;
    r_A >>= 1;
    if ((r_F & f_C) != 0) r_A |= 0x80;
    if (tmp1 != 0) r_F |= f_C; else r_F &= ~f_C;
    clock += 4;
    r_PC += 1;
    break;
...
case 0x22: // 22 SHLD addr
    ...
    break;
case 0x23: // 23 INX H
    if (++r_L > 0xFF) {r_L = 0; if (++r_H > 0xFF) r_H = 0;};
    clock += 5;
    r_PC += 1;
    break;
case 0x24: // 24 INR H
    r_F = flags[tmp2 = (r_H + 1) & 0xFF] | (r_F & f_C);
    if (((tmp2 ^ r_H) & 0x10) != 0) r_F |= f_A;
    r_H = tmp2;
    clock += 5;
    r_PC += 1;
    break;
...
case 0x27: // 27 DAA
    tmp1 = 0;
    if (((r_F & f_C) != 0) || ( r_A         > 0x99)) tmp1 |= 0x60;
    if (((r_F & f_A) != 0) || ((r_A & 0x0F) > 0x09)) tmp1 |= 0x06;
    tmp2 = r_A + tmp1;
    r_F = flags[tmp2 & 0xFF];
    if (((tmp2 ^ r_A ^ tmp1) & 0x10) != 0) r_F |= f_A;
    if (tmp2 > 0xFF) r_F |= f_C;
    r_A = tmp2 & 0xFF;
    clock += 4;
    r_PC += 1;
    break;
...
case 0x2A: // 2A LHLD addr
    ...
    break;
case 0x2B: // 2B DCX H
    if (--r_L < 0) {r_L = 0xFF; if (--r_H < 0) r_H = 0xFF;};
    clock += 5;
    r_PC += 1;
    break;
case 0x2C: // 2C INR L
    r_F = flags[tmp2 = (r_L + 1) & 0xFF] | (r_F & f_C);
    if (((tmp2 ^ r_L) & 0x10) != 0) r_F |= f_A;
    r_L = tmp2;
    clock += 5;
    r_PC += 1;
    break;
...
case 0x32: // 32 STA addr
    ...
    break;
case 0x33: // 33 INX SP
    if (r_SP == 0xFFFF) r_SP = 0; else r_SP++;
    clock += 5;
    r_PC += 1;
    break;
case 0x34: // 34 INR M
...
case 0x3A: // 3A LDA addr
    ...
    break;
case 0x3B: // 3B DCX SP
    if (r_SP != 0) r_SP--; else r_SP = 0xFFFF;
    clock += 5;
    r_PC += 1;
    break;
case 0x3C: // 3C INR A
    r_F = flags[tmp2 = (r_A + 1) & 0xFF] | (r_F & f_C);
    if (((tmp2 ^ r_A) & 0x10) != 0) r_F |= f_A;
    r_A = tmp2;
    clock += 5;
    r_PC += 1;
    break;
...
case 0x80: // 80 ADD B
    tmp2 = r_A + r_B;
    r_F = flags[tmp2 & 0xFF];
    if (((tmp2 ^ r_A ^ r_B) & 0x10) != 0) r_F |= f_A;
    if (tmp2 > 0xFF) r_F |= f_C;
    r_A = tmp2 & 0xFF;
    clock += 4;
    r_PC += 1;
...
case 0x81: // 81 ADD C
    tmp2 = r_A + r_C;
    r_F = flags[tmp2 & 0xFF];
    if (((tmp2 ^ r_A ^ r_C) & 0x10) != 0) r_F |= f_A;
    if (tmp2 > 0xFF) r_F |= f_C;
    r_A = tmp2 & 0xFF;
    clock += 4;
    r_PC += 1;
...
case 0x82: // 82 ADD D
    tmp2 = r_A + r_D;
    r_F = flags[tmp2 & 0xFF];
    if (((tmp2 ^ r_A ^ r_D) & 0x10) != 0) r_F |= f_A;
    if (tmp2 > 0xFF) r_F |= f_C;
    r_A = tmp2 & 0xFF;
    clock += 4;
    r_PC += 1;
...
case 0x83: // 83 ADD E
    tmp2 = r_A + r_E;
    r_F = flags[tmp2 & 0xFF];
    if (((tmp2 ^ r_A ^ r_E) & 0x10) != 0) r_F |= f_A;
    if (tmp2 > 0xFF) r_F |= f_C;
    r_A = tmp2 & 0xFF;
    clock += 4;
    r_PC += 1;
...
case 0x84: // 84 ADD H
    tmp2 = r_A + r_H;
    r_F = flags[tmp2 & 0xFF];
    if (((tmp2 ^ r_A ^ r_H) & 0x10) != 0) r_F |= f_A;
    if (tmp2 > 0xFF) r_F |= f_C;
    r_A = tmp2 & 0xFF;
    clock += 4;
    r_PC += 1;
...
case 0x85: // 85 ADD L
    tmp2 = r_A + r_L;
    r_F = flags[tmp2 & 0xFF];
    if (((tmp2 ^ r_A ^ r_L) & 0x10) != 0) r_F |= f_A;
    if (tmp2 > 0xFF) r_F |= f_C;
    r_A = tmp2 & 0xFF;
    clock += 4;
    r_PC += 1;
...
case 0x87: // 87 ADD A
    tmp2 = r_A + r_A;
    r_F = flags[tmp2 & 0xFF];
    if (((tmp2 ^ r_A ^ r_A) & 0x10) != 0) r_F |= f_A;
    if (tmp2 > 0xFF) r_F |= f_C;
    r_A = tmp2 & 0xFF;
    clock += 4;
    r_PC += 1;
...
case 0xE2: // E2 JPO addr
    ...
    break;
case 0xE3: // E3 XTHL
    tmp1 = do_read(r_SP); do_write(r_SP, r_L); r_L = tmp1;
    tmp1 = do_read(r_SP+1); do_write(r_SP+1, r_H); r_H = tmp1;
    clock += 18;
    r_PC += 1;
    break;
...
case 0xEA: // EA JPE addr
    ...
    break;
case 0xEB: // EB XCHG
    tmp1 = r_D; r_D = r_H; r_H = tmp1;
    tmp1 = r_E; r_E = r_L; r_L = tmp1;
    clock += 4;
    r_PC += 1;
    break;
